...
- ค่าเกินขอบเขตสูงสุด จะเกิด Overflow ทำให้ค่ากลับไปเริ่มนับจากค่าต่ำสุด (เช่น 2147483647 + 1 กลายเป็น -2147483648)
และถ้าค่าต่ำกว่าขอบเขตต่ำสุด จะเกิด Underflow ทำให้ค่ากลับไปที่ค่ามากสุดแทน
...
// double แสดงทศนิยมได้ ละเอียดกว่า float

ควรใช้ double เมื่อ ต้องการความแม่นยำสูง เช่น ในการคำนวณทางวิทยาศาสตร์ หรือ GPS
...
อักขระทุกตัวมีเลขแทน (ASCII) เช่น 'A' = 65, 'z' = 122
เราสามารถแปลง char เป็น int เพื่อดูเลข ASCII ได้
...
true แสดงเป็น 1, false แสดงเป็น 0 บน Serial Monitor
...
// ใช่, บน ESP32 long = int (ทั้งคู่เป็น 32-bit)

ถ้าจะเก็บเลขบวกขนาดใหญ่ที่สุด → ใช้ unsigned long long
...
ถ้าใส่ 256 → จะได้ 0 เพราะมันล้นขอบเขต (0–255) แล้ววนกลับไปเริ่มใหม่ (เรียกว่า overflow)
...
//  10 / 3.0 → มี float/double อยู่ → ผลลัพธ์จึงเป็นทศนิยม

10 / (int)3.0 → ทั้งสองเป็นจำนวนเต็ม → จะ ตัดเศษทิ้ง เช่น 10 / 3 = 3
...
// เมื่อต้องการ แปลงค่าจากชนิดหนึ่งไปอีกชนิดหนึ่ง เช่น float → int

เมื่อต้องการใช้ ตัวอักษรเป็นเลข (ASCII)

เมื่อต้องการ ผลลัพธ์แบบทศนิยม จากการหาร

เมื่อต้องการใช้ค่าจาก ฟังก์ชันหรือเซ็นเซอร์ที่ให้ชนิดไม่ตรงกับที่ต้องการ

เมื่อต้อง ลดขนาดข้อมูล เพื่อประหยัดหน่วยความจำ

เมื่อต้องใช้ข้อมูลกับ ไลบรารี หรืออุปกรณ์ ที่ต้องการชนิดเฉพาะ
